...
//            System.out.println("人脸检测耗时： "+(endTime - startTime)+"ms");
            ...
            for (Pair face : faces) {
                ...
                System.out.println("人脸对齐耗时： "+(endTime - startTime)+"ms");
...
//                rectF.round(rect);
    ...
    List<Recognition> recognizeStaticImage(Bitmap bitmap, Matrix matrix) {
        synchronized (this) {
            long startTime = System.currentTimeMillis();   //获取开始时间
            Pair faces[] = mtcnn.detect(bitmap);
            long endTime = System.currentTimeMillis(); //获取结束时间
            System.out.println(bitmap.getWidth()+"x"+bitmap.getHeight()+"px人脸检测耗时： "+(endTime - startTime)+"ms");
...
//                rectF.round(rect);
    ...
    public Bitmap getBitmapFromUri(ContentResolver contentResolver, Uri uri) throws Exception {
