...
public class MultiBoxTracker {
    ...
    // Maximum percentage of a box that can be overlapped by another box at pp.facerecognizer.detection time. Otherwise
...
//        logger.i("Processing %d results from %d", results.size(), timestamp);
    ...
    private void handleDetection(
        ...
        for (final TrackedRecognition trackedRecognition : trackedObjects) {
            ...
            if (intersects && intersectOverUnion > MAX_OVERLAP) {
                if (potential.first < trackedRecognition.detectionConfidence
                    ...
                    // If track for the existing object is still going strong and the pp.facerecognizer.detection score was
        ...
        for (final TrackedRecognition trackedRecognition : removeList) {
            logger.v(
                    "Removing tracked object %s with pp.facerecognizer.detection confidence %.2f, correlation %.2f",
        ...
        logger.v(
                "Tracking object %s (%s) with pp.facerecognizer.detection confidence %.2f at position %s",
                potentialObject,
